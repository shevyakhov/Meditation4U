...
class HomeActivity : AppCompatActivity() {
    ...
    private fun startingAnimation() {
        ...
        icon.animate().alpha(0.0f).setDuration(0)

        toRegistrationMain.animate().alpha(0.0f).setDuration(0)

        android.os.Handler().postDelayed({
            greetStart.animate().alpha(1.0f).setDuration(500)
            greetEnd.animate().alpha(1.0f).setDuration(500)
            loginBtn.animate().alpha(1.0f).setDuration(500)
            toRegistrationMain.animate().alpha(1.0f).setDuration(500)
            icon.animate().alpha(1.0f).setDuration(500)
